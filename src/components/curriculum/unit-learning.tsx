...
export function UnitLearning({ unit }: { unit: HiraganaUnit }) {
  ...
  useEffect(() => {
    if (screen === "lesson" && current) introduceCharacter(current.character);
  }, [current, screen]);

  const handleComplete = useCallback((result: PracticeCompletionResult) => {
    setAttempt(recordUnitAttempt(unit, result.recognitionCorrect, result.typingCorrect));
    setScreen("result");
  }, [unit]);
  ...
  if (!accessible) return <section className="mx-auto max-w-xl py-24 text-center"><p className="text-sm font-bold text-[var(--kaku-purple)]">Unidad {unit.id}</p><h1 className="mt-5 text-4xl font-extrabold tracking-[-0.05em]">Todavía no está disponible</h1><p className="mt-4 text-[var(--muted)]">Completa primero la unidad anterior.</p><ButtonLink href="/hiragana" className="mt-8">Volver a las unidades</ButtonLink></section>;
  ...
  if (screen === "result" && attempt) {
    ...
    return <section className="mx-auto max-w-2xl py-16 text-center"><p className="text-sm font-bold text-[var(--kaku-purple)]">Unidad {unit.id}</p><h1 className="mt-5 text-4xl font-extrabold tracking-[-0.05em] sm:text-5xl">{attempt.passed ? "Unidad aprendida" : "Vamos a repasarlos otra vez"}</h1><p className="mt-5 text-lg text-[var(--muted)]">{attempt.recognitionCorrect} / {unit.characters.length} reconocidos · {attempt.typingCorrect} / {unit.characters.length} recordados</p><div className="mx-auto my-10 flex size-36 items-center justify-center rounded-full bg-[var(--yuzu)] text-3xl font-extrabold text-[var(--sumi)]">{percentage}%</div><div className="grid gap-3 sm:grid-cols-2">{attempt.passed ? <ButtonLink href="/hiragana">Continuar aprendiendo</ButtonLink> : <Button type="button" onClick={() => { setPracticeKey((key) => key + 1); setScreen("practice"); }}>Practicar de nuevo</Button>}<ButtonLink href="/hiragana" variant="neutral" className="border border-[var(--border)]">Volver a las unidades</ButtonLink></div></section>;
  }

  return (
    <section className="mx-auto max-w-2xl py-10 sm:py-16">
      <header className="flex items-center justify-between"><Link href="/hiragana" className="min-h-11 py-3 text-sm font-semibold text-[var(--muted)] hover:text-[var(--sumi)]">← Unidades</Link><span className="text-sm font-semibold tabular-nums text-[var(--muted)]">{characterIndex + 1} / {unit.characters.length}</span></header>
      <ProgressBar className="mt-3 text-[var(--kaku-purple)]" value={characterIndex + 1} max={unit.characters.length} label="Progreso de la lección" />
      <Card key={current.character} className="mt-8 animate-[question-in_.2s_ease-out] border border-[var(--border)] p-7 text-center shadow-[0_24px_80px_rgba(55,21,143,.06)] sm:p-12">
        <p className="text-sm font-bold text-[var(--kaku-purple)]">Unidad {unit.id} · Conoce los kana</p>
