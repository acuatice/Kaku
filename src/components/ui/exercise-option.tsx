...
export function ExerciseOption({ state = "idle", children, className = "", ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { state?: ExerciseOptionState; children: ReactNode }) {
  return <button type="button" className={`min-h-[4.25rem] touch-manipulation rounded-[1.25rem] px-4 text-lg font-bold transition duration-150 focus-visible:outline-2 focus-visible:outline-offset-2 sm:min-h-18 ${states[state]} ${className}`} {...props}>{children}</button>;
}
